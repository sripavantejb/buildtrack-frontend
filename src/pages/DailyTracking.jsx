...
export default function DailyTracking({ project }) {
  ...
  const donutColors = ['#f54e00', '#1f8a65', '#c08532', '#EF4444'];
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <div className="rounded-lg border border-hairline bg-surface-card p-4">
          <p className="text-[9px] font-bold text-muted-soft uppercase tracking-wider">Today's Cost</p>
          <h3 className="mt-1 text-base font-black text-ink">{formatRupees(latestLog.todayCost)}</h3>
          <p className="mt-1 flex items-center gap-1 text-[9px] font-bold text-success">
        ...
        <div className="rounded-lg border border-hairline bg-surface-card p-4">
          <p className="text-[9px] font-bold text-muted-soft uppercase tracking-wider">Materials Used</p>
          <h3 className="mt-1 text-base font-black text-ink">{latestLog.materialsUsed} Units</h3>
          <p className="mt-1 flex items-center gap-1 text-[9px] font-bold text-success">
        ...
        <div className="rounded-lg border border-hairline bg-surface-card p-4">
          <p className="text-[9px] font-bold text-muted-soft uppercase tracking-wider">Work Progress</p>
          <h3 className="mt-1 text-base font-black text-ink">{latestLog.workProgress}%</h3>
          <p className="mt-1 flex items-center gap-1 text-[9px] font-bold text-success">
            <TrendingUp className="h-3.5 w-3.5" />
        ...
        <div className="rounded-lg border border-hairline bg-surface-card p-4">
          <p className="text-[9px] font-bold text-muted-soft uppercase tracking-wider">Labour Present</p>
          <h3 className="mt-1 text-base font-black text-ink">{latestLog.labourPresent} Workers</h3>
          <p className="mt-1 text-[9px] font-bold text-timeline-done">
            {latestLog.attendance?.absent || 0} absent today
          </p>
        </div>

        <div className="rounded-lg border border-hairline bg-surface-card p-4">
          <p className="text-[9px] font-bold text-muted-soft uppercase tracking-wider">Working Hours</p>
          <h3 className="mt-1 text-base font-black text-ink">{latestLog.workingHours} hrs</h3>
          <p className="mt-1 text-[9px] font-semibold text-muted-soft">Goal: 9.0 hrs</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-lg border border-hairline bg-surface-card p-5 flex flex-col justify-between">
          <h4 className="text-xs font-semibold text-ink mb-4">Material Usage Today</h4>
          
          <div className="flex-1 overflow-x-auto">
            <table className="w-full text-left border-collapse text-[11px] font-medium text-body">
              <thead>
              ...
              <tbody className="divide-y divide-hairline-soft">
                {latestLog.materialUsageToday?.map((usage) => (
                  <tr key={usage.material} className="hover:bg-canvas/50">
                    <td className="py-2.5 font-semibold text-ink">{usage.material}</td>
                    <td className="py-2.5 text-right font-semibold">{usage.used}</td>
                    <td className="py-2.5 text-center text-muted-soft font-semibold">{usage.unit}</td>
                    <td className="py-2.5 text-right font-semibold text-ink">{formatRupees(usage.cost)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Area-Wise Usage */}
        <div className="rounded-lg border border-hairline bg-surface-card p-5 flex flex-col justify-between">
          <h4 className="text-xs font-semibold text-ink mb-4">Area-Wise Usage</h4>
            ...
            {latestLog.areaWiseUsage?.map((area) => (
              <div key={area.area} className="space-y-1">
                <div className="flex items-center justify-between text-[10px] font-bold">
                  ...
                  <span className="text-muted">{area.quantity} Bags <span className="font-semibold text-muted-soft">Cement</span></span>
                </div>
        ...
        <div className="rounded-lg border border-hairline bg-surface-card p-5 flex flex-col justify-between">
          <h4 className="text-xs font-semibold text-ink">Daily Expenses Breakdown</h4>
          ...
          <div className="relative flex h-36 items-center justify-center my-1">
            ...
            <div className="absolute flex flex-col items-center">
              <span className="text-[8px] font-bold text-muted-soft uppercase tracking-wider">Total</span>
              <span className="text-xs font-black text-ink mt-0.5">{formatRupees(latestLog.todayCost)}</span>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-1.5 border-t border-hairline-soft pt-3 text-[10px] font-semibold">
            {latestLog.expensesBreakdown?.map((d, index) => (
              <div key={d.name} className="flex items-center justify-between">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-muted-soft text-[9px] truncate max-w-[60px]">{d.name}</span>
                </div>
                <span className="text-ink font-bold">{d.percentage}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-lg border border-hairline bg-surface-card p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-timeline-read/20 text-timeline-read">
              <Coins className="h-4.5 w-4.5" />
            </div>
            <div>
              <h5 className="text-xs font-semibold text-ink">Today's Expenses</h5>
              <p className="text-[9px] text-muted-soft mt-0.5">Total spend generated today</p>
            </div>
          </div>
          <span className="text-xs font-black text-ink">{formatRupees(latestLog.todayCost)}</span>
        </div>

        {/* Labour Present */}
        <div className="rounded-lg border border-hairline bg-surface-card p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-timeline-grep/20 text-success">
              <Hammer className="h-4.5 w-4.5" />
            </div>
            <div>
              <h5 className="text-xs font-semibold text-ink">Labour & Attendance</h5>
              <p className="text-[9px] text-muted-soft mt-0.5">Workers checking in today</p>
            </div>
          </div>
          <span className="text-xs font-black text-ink">{latestLog.labourPresent} Present</span>
        </div>

        {/* Equipment Usage */}
        <div className="rounded-lg border border-hairline bg-surface-card p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-canvas-soft text-primary">
              <Truck className="h-4.5 w-4.5" />
            </div>
            <div>
              <h5 className="text-xs font-semibold text-ink">Equipment Usage</h5>
              <p className="text-[9px] text-muted-soft mt-0.5">Machinery active onsite</p>
            </div>
          </div>
          <span className="text-xs font-black text-ink">{latestLog.equipment?.active || 5} Active</span>
        </div>

        {/* Issues & Delays */}
        <div className="rounded-lg border border-hairline bg-surface-card p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-canvas-soft text-error">
              <AlertOctagon className="h-4.5 w-4.5" />
            </div>
            <div>
              <h5 className="text-xs font-semibold text-ink">Issues & Delays</h5>
              <p className="text-[9px] text-muted-soft mt-0.5">Unresolved alerts on site</p>
            </div>
          </div>
          <span className={`text-xs font-black ${latestLog.issues?.length > 0 ? 'text-error' : 'text-ink'}`}>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4">
          <div className="w-full max-w-md rounded-lg border border-hairline-soft bg-surface-card p-6">
            ...
            <form onSubmit={handleCreateEntry} className="mt-4 space-y-3.5">
              <div className="grid grid-cols-2 gap-3.5">
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-ink mb-1">Labour Count</label>
                  <input 
                    type="number" 
                    required
              ...
              <div className="grid grid-cols-2 gap-3.5">
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-ink mb-1">Hours Worked</label>
                  <input 
                    type="number" 
                    step="0.5"
                    required
              ...
              <div className="border-t border-hairline-soft pt-3">
                <h4 className="text-[10px] font-bold text-muted uppercase mb-2">Material Usage Quantity</h4>
                  ...
                  <div className="grid grid-cols-2 gap-3.5">
                    <div>
                      ...
                      <input 
                        ...
                        placeholder="e.g. 50"
                        className="w-full rounded border border-hairline bg-surface-card px-2 py-1 text-xs text-ink focus:border-primary focus:outline-none"
                      />
                    </div>
                    <div>
                      <label className="block text-[8px] font-semibold text-body mb-1">Block B</label>
                      <input 
                        type="number" 
                    ...
                    <div>
                      <label className="block text-[8px] font-semibold text-body mb-1">Block C (Roof)</label>
                      <input 
                        type="number" 
                    ...
                    <div>
                      <label className="block text-[8px] font-semibold text-body mb-1">Site Office</label>
                      <input 
                        type="number" 
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <input 
                      ...
                      placeholder="e.g. 4.2"
                      className="w-full rounded border border-hairline px-2 py-1 text-xs text-ink focus:border-primary focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[9px] font-semibold text-body mb-1">Sand (Loads)</label>
                    <input 
                      type="number" 
                  ...
                  <div>
                    <label className="block text-[9px] font-semibold text-body mb-1">Bricks (Nos)</label>
                    <input 
                      type="number" 
              ...
              <div>
                <label className="block text-[10px] font-semibold text-ink mb-1">Site Issues / Delays</label>
                <input 
                  type="text" 
          ...
          <div className="w-full max-w-lg rounded-lg border border-hairline-soft bg-surface-card p-6 flex flex-col max-h-[85vh]">
            <div className="flex items-center justify-between pb-3 border-b border-hairline">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-ink">Critical Project Alerts</h3>
                  <p className="text-[10px] text-muted-soft font-semibold mt-0.5">Please review and acknowledge these alerts to proceed</p>
                </div>
              </div>
              <button 
                onClick={() => setShowAlertsModal(false)}
                className="text-muted-soft hover:text-body text-sm font-bold p-1 rounded-full hover:bg-canvas transition-colors"
            ...
            <div className="mt-4 flex-1 overflow-y-auto space-y-3 pr-1 py-1">
              {alerts.map((alert) => {
                let colorClass = "bg-timeline-read/20 border-hairline text-ink";
                let typeLabel = "Info";
                if (alert.type === "Critical" || alert.type === "Danger") {
                  colorClass = "bg-canvas-soft/70 border-hairline text-red-800";
                  typeLabel = "Critical";
                } else if (alert.type === "Warning") {
                  colorClass = "bg-timeline-thinking/20/70 border-amber-100 text-amber-800";
                  typeLabel = "Warning";
                } else if (alert.type === "Success") {
                  colorClass = "bg-timeline-grep/20/70 border-hairline text-green-800";
                  typeLabel = "Success";
                }
                ...
                return (
                  <div 
                    key={alert.id} 
                    className={`flex items-start justify-between gap-3 p-4 rounded-lg border ${colorClass} transition-all duration-200 hover:scale-[1.01]`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-[8px] uppercase tracking-wider font-normal px-1.5 py-0.5 rounded bg-surface-card/60">
                            {typeLabel}
                          </span>
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-full p-1 hover:bg-black/5 transition-colors text-muted hover:text-ink flex items-center justify-center font-bold"
                      title="Dismiss alert"
                    >
            ...
            <div className="mt-6 pt-3 border-t border-hairline-soft flex justify-between items-center">
              <span className="text-[9px] font-bold text-muted-soft">
